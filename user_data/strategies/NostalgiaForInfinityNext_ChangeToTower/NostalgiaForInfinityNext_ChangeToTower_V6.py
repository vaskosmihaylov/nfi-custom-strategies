...
class NostalgiaForInfinityNext_ChangeToTower_V6(IStrategy):
    ...
    # ROI table: tightened for faster profit capture & reduced exposure
    minimal_roi = {
        "0": 0.10,
        "20": 0.05,
        "45": 0.03,
    ...
    stoploss = -0.07  # Tightened from -0.10, moderate compromise

    # Trailing stoploss - moderately tighter than original
    trailing_stop = True
    trailing_only_offset_is_reached = True
    trailing_stop_positive = 0.008  # Moderate: between original 0.01 and previous 0.005
    trailing_stop_positive_offset = 0.02  # Moderate: between original 0.03 and previous 0.015

    use_custom_stoploss = True  # Enable tiered custom stoploss
    ...
    exit_profit_only = True  # Restored: strategy relies on hold-for-recovery behavior
    ...
    buy_params = {
        ...
        "buy_condition_9_enable": False,  # Disabled: minimal protection filters
        "buy_condition_10_enable": True,
        "buy_condition_11_enable": False,  # Disabled: weak protection, RSI-only
        ...
        "buy_condition_20_enable": False,  # Disabled: unprotected entries
        ...
        "buy_condition_35_enable": False,  # Disabled: no safe_dips/pump/sma200
        "buy_condition_36_enable": False,  # Disabled: no safe_dips/pump/sma200
        "buy_condition_37_enable": False,  # Disabled: no safe_dips/pump/sma200
        "buy_condition_38_enable": False,  # Disabled: no safe_dips/pump/sma200
        "buy_condition_39_enable": False,  # Disabled: no safe_dips/pump/sma200
        "buy_condition_40_enable": False,  # Disabled: no safe_dips/pump/sma200
        #############
    }
    ...
    def sell_dec_main(self, current_profit: float, last_candle) -> tuple:
        if (self.sell_custom_dec_profit_max_1.value > current_profit >= self.sell_custom_dec_profit_min_1.value) and (
            last_candle["sma_200_dec_20"]
        ):
            return True, "signal_profit_d_1"
        elif (
            self.sell_custom_dec_profit_max_2.value > current_profit >= self.sell_custom_dec_profit_min_2.value
    ...
    def sell_trail_main(self, current_profit: float, last_candle, max_profit: float) -> tuple:
        if (
            (self.sell_trail_profit_max_1.value > current_profit >= self.sell_trail_profit_min_1.value)
            and (self.sell_trail_rsi_min_1.value < last_candle["rsi_14"] < self.sell_trail_rsi_max_1.value)
            and (max_profit > (current_profit + self.sell_trail_down_1.value))
            and (last_candle["moderi_96"] == False)
        ):
            return True, "signal_profit_t_1"
        elif (
            (self.sell_trail_profit_max_2.value > current_profit >= self.sell_trail_profit_min_2.value)
            and (self.sell_trail_rsi_min_2.value < last_candle["rsi_14"] < self.sell_trail_rsi_max_2.value)
            ...
            (self.sell_trail_profit_max_3.value > current_profit >= self.sell_trail_profit_min_3.value)
            and (max_profit > (current_profit + self.sell_trail_down_3.value))
            and (last_candle["sma_200_dec_20_1h"])
        ):
            return True, "signal_profit_t_3"
        elif (
            (self.sell_trail_profit_max_4.value > current_profit >= self.sell_trail_profit_min_4.value)
    ...
    def sell_duration_main(
        ...
        if (
            ...
            self.sell_custom_long_profit_min_1.value < current_profit < self.sell_custom_long_profit_max_1.value
        ) and (current_time - timedelta(minutes=self.sell_custom_long_duration_min_1.value) > trade.open_date_utc):
    ...
    def sell_under_min(self, current_profit: float, last_candle) -> tuple:
        if (last_candle["moderi_96"]) == False:
            ...
            if (
                ...
                and (last_candle["rsi_14"] > last_candle["rsi_14_1h"] + self.sell_custom_profit_under_rsi_diff_1.value)
            ):
            ...
            if (
                ...
                and (last_candle["rsi_14"] > last_candle["rsi_14_1h"] + self.sell_custom_profit_under_rsi_diff_2.value)
            ):
    ...
    def sell_stoploss(self, current_profit: float, last_candle, previous_candle_1) -> tuple:
        # ATR-based stoplosses: tightened moderately from original (-8/-12/-16/-20)
        if -0.06 <= current_profit < -0.03:
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        """
        Tiered custom stoploss to prevent catastrophic losses.
        Tightens the stop progressively as losses deepen.
        """
        # In profit: let trailing_stop settings handle it (don't conflict)
        if current_profit > 0.04:
            return -0.02  # Lock at least 2% once 4% is reached
        elif current_profit > 0.0:
            return -1  # Use default stoploss / trailing while building profit
        elif current_profit > -0.04:
            # Loss <4%: let default -0.07 handle it, allow recovery
            return -1
        elif current_profit > -0.06:
            # Loss 4-6%: tighten slightly
            return -0.06
        else:
            # Loss > 6%: near hard stop, let it trigger
            return -0.05
    ...
    def custom_exit(
        ...
        # Time-based exit: only cut stale trades losing > 3% after 12 hours
        if (current_profit < -0.03) and (current_time - timedelta(minutes=720) > trade.open_date_utc):
            return "exit_stale_loss_12h"
    ...
    def informative_1h_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Ichimoku
        ichi = ichimoku(
            informative_1h, conversion_line_period=20, base_line_periods=60, laggin_span=120, displacement=30
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.res_timeframe != "none":
            ...
            dataframe.rename(
                columns=lambda s: s + "_{}".format(self.res_timeframe) if "resample_" in s else s, inplace=True
            )
            dataframe.rename(
        ...
        for index in self.buy_protection_params:
            ...
            if self.buy_params["buy_condition_" + str(index) + "_enable"]:
                ...
                if global_buy_protection_params["close_above_ema_fast"]:
                    item_buy_protection_list.append(
                        dataframe["close"]
                        > dataframe[f"ema_{global_buy_protection_params['close_above_ema_fast_len']}"]
                    )
                if global_buy_protection_params["close_above_ema_slow"]:
                    item_buy_protection_list.append(
                        dataframe["close"]
                        > dataframe[f"ema_{global_buy_protection_params['close_above_ema_slow_len']}_1h"]
                    )
                if global_buy_protection_params["sma200_rising"]:
                    item_buy_protection_list.append(
                        dataframe["sma_200"]
                        > dataframe["sma_200"].shift(int(global_buy_protection_params["sma200_rising_val"]))
                    )
        ...
        pm_arr[i] = (
            ...
            if (pm_arr[i - 1] == final_ub[i - 1] and mavalue[i] <= final_ub[i])
                ...
                if (pm_arr[i - 1] == final_ub[i - 1] and mavalue[i] > final_ub[i])
                    ...
                    else final_ub[i] if (pm_arr[i - 1] == final_lb[i - 1] and mavalue[i] < final_lb[i]) else 0.00
                )
            )
        )
