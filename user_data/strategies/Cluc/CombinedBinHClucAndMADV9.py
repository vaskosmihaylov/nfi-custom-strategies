...
class CombinedBinHClucAndMADV9(IStrategy):
    ...
    sell_params = {
        #############
        # Enable/Disable short conditions
        "short_condition_1_enable": True,
        "short_condition_2_enable": True,
        "short_condition_3_enable": True,
        "short_condition_4_enable": True,
        "short_condition_5_enable": True,
    }
    ...
    # Short condition toggles
    short_condition_1_enable = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=False, load=True
    )
    short_condition_2_enable = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=False, load=True
    )
    short_condition_3_enable = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=False, load=True
    )
    short_condition_4_enable = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=False, load=True
    )
    short_condition_5_enable = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=False, load=True
    )

    # Short params
    sell_bb20_close_bbupperband_safe_1 = DecimalParameter(
        0.9, 1.3, default=1.01, space="sell", optimize=False, load=True
    )
    sell_bb20_close_bbupperband_safe_2 = DecimalParameter(
        0.9, 1.3, default=1.018, space="sell", optimize=False, load=True
    )
    ...
    sell_rsi_1 = DecimalParameter(60.0, 90.0, default=72.0, space="sell", decimals=1, optimize=False, load=True)
    ...
    def custom_stoploss(
        ...
        # Works for both long and short positions via current_profit abstraction.
        ...
        if current_profit > 0:
            ...
            if trade_time_50 > trade.open_date_utc:
                ...
                try:
                    ...
                    if trade.is_short:
                        # Short: are we still pumping? (price rose 1.5% above candle open)
                        if current_rate * 0.985 > candle["open"]:
                            return 0.01
                    else:
                        # Long: are we still sinking?
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ===================================================================
        # Short entries: overbought conditions (inverted long logic)
        # ===================================================================
        dataframe.loc[
            (
                self.short_condition_1_enable.value
                &
                # Price BELOW ema_200 = downtrend (inverted: long requires above)
                (dataframe["close"] < dataframe["ema_200"])
                & (dataframe["close"] < dataframe["ema_200_1h"])
                &
                # Price above upper BB = overbought (inverted: long buys below lower BB)
            ...
            | (
                self.short_condition_2_enable.value
                & (dataframe["close"] < dataframe["ema_200"])
            ...
            | (
                self.short_condition_3_enable.value
                &
                # RSI 1h overbought (inverted: long buys when 1h RSI is low)
            ...
            | (
                self.short_condition_4_enable.value
                &
                # MACD bearish: ema_12 > ema_26 (inverted from long where ema_26 > ema_12)
                (dataframe["close"] < dataframe["ema_200"])
                & (dataframe["close"] < dataframe["ema_200_1h"])
            ...
            | (
                self.short_condition_5_enable.value
                &
                # SSL down trend + high RSI divergence (inverted from long condition 10)
                (dataframe["close"] > dataframe["sma_5"])
                & (dataframe["ssl_down_1h"] > dataframe["ssl_up_1h"])
                & (dataframe["ema_200_1h"] > dataframe["ema_50_1h"])
                & (dataframe["rsi"] > (100 - (dataframe["rsi_1h"] - 43.276)))
                & (dataframe["volume"] > 0)
            ),
            "enter_short",
        ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Exit long: close above mid BB
        dataframe.loc[
            ((dataframe["close"] > dataframe["bb_middleband"] * 1.01) & (dataframe["volume"] > 0)), "exit_long"
        ] = 1

        # Exit short: close below mid BB (inverted from exit_long)
        dataframe.loc[
            ((dataframe["close"] < dataframe["bb_middleband"] * 0.99) & (dataframe["volume"] > 0)), "exit_short"
        ] = 1
